...
export async function GET(request: Request) {
  ...
  if (!user) {
    ...
    return NextResponse.json({ users: { ...users, ...tradeBlockedUsers }, tradeClose: tradeCloseDashboard });
  }
  ...
  const status = manualBlocked || tradeBlocked ? "BLOCK" : "OK";

  return NextResponse.json({
    user,
    status,
    isBlocked: status === "BLOCK",
    blockedReason: tradeBlocked ? "tradeClose" : manualBlocked ? "manual" : undefined,
    unresolvedCount: tradeClose.unresolvedCount,
