  ...
  return (
    <section className="min-w-0 overflow-hidden rounded-[20px] border border-[#e9eef6] bg-white p-4 shadow-[0_6px_16px_rgba(15,23,42,0.032)]">
      <div className="flex min-w-0 items-start justify-between gap-4">
        ...
        <span className="shrink-0 rounded-full bg-[#fff1f2] px-3 py-1 text-[12px] font-[950] text-[#ef4444]">{issues.length}건</span>
      </div>
      ...
      <div className="mt-3 grid min-w-0 grid-cols-3 gap-2">
        {collectionItems.map((item) => (
          <div key={item.label} className="h-[100px] min-w-0 overflow-hidden rounded-[16px] border border-[#edf2f8] bg-[#fbfcff] p-3 shadow-[0_4px_10px_rgba(15,23,42,0.022)]">
            ...
            <p className={`mt-1 truncate text-[24px] font-[950] leading-none tracking-[-0.03em] ${item.status === "needCheck" ? "text-[#ef4444]" : "text-[#111827]"}`}>
              {item.count}
            </p>
      ...
      <button onClick={() => (window.location.href = "/collections")} className="mt-3 h-[38px] w-full rounded-[14px] border border-[#e9eef6] bg-[#f8fbff] text-[13px] font-[950] text-[#2563eb] shadow-[0_4px_10px_rgba(15,23,42,0.025)] transition hover:bg-[#eef5ff]">
        수금관리 바로가기
      </button>
    </section>
  );
