...
export function CollectionCheckCard() {
  ...
  const collectionItems = [
    ...
    { label: "수금매칭 보류", count: `${partialCount}건`, sub: "부분입금 확인", icon: Banknote, tone: "orange" },
  ...
  return (
    <section className="min-w-0 overflow-hidden rounded-[20px] border border-[#e5eaf3] bg-white p-4 shadow-[0_8px_20px_rgba(15,23,42,0.045)]">
      <div className="flex min-w-0 items-start justify-between gap-4">
        ...
        <span className="shrink-0 rounded-full bg-[#eaf2ff] px-3 py-1 text-[12px] font-[950] text-[#2563eb]">수금 이슈</span>
      </div>
      ...
      <button onClick={() => (window.location.href = "/collections")} className="mt-3 h-[38px] w-full rounded-[14px] border border-[#cfe2ff] bg-[#eef6ff]/85 text-[13px] font-[950] text-[#2563eb] shadow-[0_8px_18px_rgba(37,99,235,0.06)] transition hover:bg-[#e3f0ff]">
        수금관리 바로가기
      </button>
    </section>
  );
