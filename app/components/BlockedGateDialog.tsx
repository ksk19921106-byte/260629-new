...
export function BlockedGateDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0d1b3e]/30 px-4">
      <div className="w-[500px] rounded-[22px] border border-red-100 bg-white shadow-[0_22px_70px_rgba(15,35,70,0.22)]">
        ...
        <div className="px-6 py-5">
          <div className="rounded-2xl bg-[#fff7f7] px-4 py-3 text-[13px] font-[700] leading-6 text-[#34496b]">
            ...
            <p className="mt-2 font-[900] text-red-600">미종료 거래가 0건이 되면 요청 진입이 가능합니다.</p>
          </div>
        </div>

        <div className="flex justify-end gap-2 border-t border-[#e3ebf6] px-6 py-4">
          <button onClick={openTrades} className="h-10 rounded-xl bg-red-600 px-5 text-[13px] font-[900] text-white">
            미종료 거래 확인하기
          </button>
          <button onClick={openIki} className="h-10 rounded-xl bg-[#075bdc] px-5 text-[13px] font-[900] text-white">
            IKI 월마감 바로가기
          </button>
